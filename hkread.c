...
printf("%d\n",state);
	...
	switch (state)
	{
	case 1:     if (result[i]==(char)0xf0){state=2;k=14;}; //awaiting f0
		break;
	case 2: 	
		if (!k--) state=3;
//	  fputc(result[i], stderr);
		  if (result[i]>(char)31) i++;

		break;
	case 3:      if (result[i]==(char)0xf1){state=4;k=14;}; //awaiting f1
                break;
	case 4:     
                if (!k--) state=5;
//	fputc(result[i], stderr);
		if (result[i]>(char)31) i++;
                ...
                break;
 	case 5:      if (result[i]==(char)0xf2){state=6;k=14;}; //awaiting f1
                break;
        case 6:     
                if (!k--) state=7;
		if (result[i]>(char)31) i++;
 ...
 //  while (1)
	{
       ...
       if ( n=readport(fd,buf)>0) printf("%s\n",buf);
		}
