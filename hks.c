...
{ 
 ...
 unsigned char checkword1;
 unsigned char checkword2;
} packetRC = 
{"PCSEND", 0x2, 0x4, {0x80, 0x70, 0xc0, 0x3f},0x00,0x00};

//char data [4] = ""; //create the null. 

for (loop =0; loop < 4; loop++)  //build the command string. 
        packetRC.data[loop] = command_list[cmd].command_hex[loop];  

        packetRC.checkword1 = packetRC.data[0] ^ packetRC.data[2];
        packetRC.checkword2 = packetRC.data[1] ^ packetRC.data[3];
                 
        if (!writeport(fd, (char *) &packetRC, sizeof(packetRC))){ 
                printf("write failed\n");  //something went horribly wrong..  
                return -1; 
                } 
printf("wrote %d bytes\n",sizeof(packetRC));  //something went horribly wrong..                   
                 
return 0;   
} 


int send_text(char * line1, char * line2, char * line3) 
{ 
struct a  
 { 
 char transmission[6];  
...
char debug [58];
 ...
 packetRaw packetRC = 
{"PCSEND", 0x3, 48, 0xf0,"",00,0xf1,"",00,0xf2,"",00,0x00,0x00};

strncpy(packetRC.data.line1,line1,14);
strncpy(packetRC.data.line2,line2,14);
strncpy(packetRC.data.line3,line3,14);

for (loop =0; loop < 24; loop++)  //build the command string. 
      {
        packetRC.raw.checkword1 = packetRC.raw.checkword1 ^ packetRC.raw.data[loop*2];
        packetRC.raw.checkword2 = packetRC.raw.checkword2 ^ packetRC.raw.data[loop*2+1];
       }             
        if (!writeport(fd, (char *) &packetRC, sizeof(packetRC))){ 
                printf("write failed\n");  //something went horribly wrong..  
                return -1; 
                } 
printf("wrote %d bytes: %s48\n",sizeof(packetRC),packetRC.debug);  //something went horribly wrong..                   
...
while (0 != 1)  //main loop, run forever... ^c to force quit
        {
                ...
                for (int indx =0;indx < 255; indx++)  //loop through, see if it matches anything
                {
//		short sCmd[16];
		key =getch();  //get a keystroke from the keyboard................
...
//        packetRC.data[loop] = command_list[cmd].command_hex[loop];..
                    ...
                    //    sCmd[0]=0x80;  // load the command to be sent.
                    //    sCmd[1]=0x70;
                        packetRC.data[2]=indx;
                        packetRC.data[3]=0x1ef-packetRC.data[0]-packetRC.data[1]-packetRC.data[2];

        packetRC.checkword1 = packetRC.data[0] ^ packetRC.data[2];
        packetRC.checkword2 = packetRC.data[1] ^ packetRC.data[3];

                    //    sCmd[4]=0x00;
			printf("%x-%x-%x-%x\n",(int)packetRC.data[0],(int)packetRC.data[1],(int)packetRC.data[2],(int)packetRC.data[3]);
        ...
        show_commands(); 
        send_text("LINE1 LINE1 OK","LINE2 LINE2 OK","LINE3 LINE3 OK");
        return 0; 
