...
{
printf("\n%d-",state);
fputc(result[i+j], stdout);

	switch (state)
	{
	case 1:     if (result[i+j]==(char)0xf0){state=2;}; //awaiting f0
		break;
	case 2: 	
         if (result[i+j]==(char)0xf2) state =3;
         if (result[i+j]<(char)32 || result[i+j]>(char)0xe9) result [i+j]=' ';
         fputc(result[i+j], stderr);
		break;
	case 3:	result[i+j]=0;
        printf("%s\n",result[4]);
		return i+j;
